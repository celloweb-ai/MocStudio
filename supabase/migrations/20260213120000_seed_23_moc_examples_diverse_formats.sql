...
BEGIN
  ...
  IF v_user_id IS NULL THEN
    RAISE NOTICE 'Skipping MOC example seed: no users found in auth.users.';
    RETURN;
  END IF;
  ...
  VALUES
    ...
    ('Laboratory & QA Center', 'FAC-LAB', 'Laboratory', 'Quality Campus', v_user_id, 'active')
  ...
  FOR rec IN
    ...
    FROM (
      VALUES
        ...
        ('FAC-PAD','Install Auto Case Erector Guard Interlocks','Add interlocks and guarding feedback for case erector machine.','Recent near misses indicate inadequate safeguarding verification.','equipment_modification','high','submitted',false,'8 days',ARRAY['Case erector CE-07','Safety PLC'],ARRAY['Packaging hall'],3,4,'high','Validate stop category and prove interlock diagnostics before handover.',false,12,9,'Case Erector Safety Interlock CEI-07'),
        ('FAC-NPP','Revise Isolation Matrix for Maintenance PTW','Update lockout-tagout matrix and permit approval path.','Audit identified gaps between isolation plans and permit system.', 'procedure_change','medium','approved',false,'4 days',ARRAY['PTW system','Isolation procedures'],ARRAY['Entire plant'],2,4,'medium','Conduct supervisor workshop before enforcement.',false,9,8,'Isolation Matrix Register IMR-2026'),
        ...
        ('FAC-LAB','Migrate LIMS Result Approval Workflow','Move paper-based QA release to digital signatures in LIMS.','Reduce release delays and improve traceability of test approvals.','software_change','high','submitted',false,'16 days',ARRAY['LIMS','QA release workflow'],ARRAY['Quality control lab'],3,4,'high','Run dual workflow for two weeks before cutover.',false,21,14,'LIMS Validation Server LIMS-SRV-02'),
        ('FAC-LAB','Install Portable Gas Chromatograph Docking Station','Add docking/charging point with sample metadata upload.','Portable analyzer data is currently stored manually in spreadsheets.','equipment_addition','medium','under_review',false,'6 days',ARRAY['Laboratory analyzers','Data historian'],ARRAY['Analytical bench 4'],2,3,'medium','Define calibration check at every docking event.',false,8,7,'GC Docking Station GCD-01'),
        ('FAC-LAB','Requalify Raw Material Sampling SOP','Create simplified and photo-guided SOP format for sampling points.','Onboarding feedback showed ambiguity in current text-only instruction.','procedure_change','low','draft',false,'5 days',ARRAY['Sampling SOP','Warehouse QA'],ARRAY['Raw material bay'],1,2,'low','Add bilingual quick cards and supervisor signoff.',false,10,10,'Sampling SOP Terminal SOP-QA-04')
    ) AS t(
      facility_code,
      moc_title,
      moc_description,
  ...
  RAISE NOTICE 'MOC example ensure complete. Inserted % missing records (target catalog: 23).', v_inserted_count;
END;
$$;
